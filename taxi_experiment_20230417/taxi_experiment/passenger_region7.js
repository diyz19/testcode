...
//乘客发出调度请求
async function manageVehicleByRegion7() {

	for (let i = 0; i < Math.min(passengerPositionList.length, passengerIdList.length); i++) {
		setTimeout(() => {
			passengerUnit(i)
...
async function passengerUnit(i) {
	let passengerId = passengerIdList[i];
	let isboard = false;
	let passengerMessage = {};

	await trafficContract.methods.initPassenger(passengerId, web3.utils.asciiToHex(passengerPositionList[i].start)).send({ from: passengerId, gas: 5000000, position: "wx411111111111", txtime: 278000 }).then(function (error, result) {
	...
	trafficContract.events.routeEvent(function (error, event) {
		...
		if (event.returnValues.passengerId.slice(0, 42) == passengerId.toLowerCase()) {
			...
			if (isboard == false) {
				isboard = true
				trafficContract.methods.confirmBoard(passengerMessage.vehicleId).send({ from: passengerId, gas: 5000000, position: "wx411111111111", txtime: 278000 }).then(function (result) {
					console.log("乘客确认上车");
				})
			} else {
	...
	let regionVehicles = [];
	let neighbourRegion = getNeighbour(passengerPositionList[i].start.slice(0, 7));
	neighbourRegion.unshift(passengerPositionList[i].start.slice(0, 7));
	let regionTasks = [];
...
async function getVehicleByRegion(passengerId, positionGeohash, regionVehicles, passengerMessage, count) {
	...
	trafficContract.methods.getVehicleByRegion(
		web3.utils.asciiToHex(positionGeohash), regionVehicles
	).call(
		...
		async function (result1) {
			...
			trafficContract.methods.setVehicleStatus(result1[1], passengerId, web3.utils.asciiToHex(positionGeohash)).send({ from: passengerId, gas: 5000000, position: "wx411111111111", txtime: 278000 }).then(function (result2) {
				...
				if (countNum == passengerIdList.length) {
					let gname = "./passengerResult/region7_passenger.json";
	...
	await web3.eth.getAccountByRegion(region).then(function (result) {
		if (result != null) {
			...
			for (let j = 0; j < resultVehicles.length; j++) {
				if (resultVehiclesTime[j] > (startTime - 60000)) {
