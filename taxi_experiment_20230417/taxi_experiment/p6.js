...
async function manageVehicleByRegion6(){
    ...
    for(let i = 0; i < Math.min(passengerIdList.length, passengerPositionList.length); i++){
		setTimeout(() => {
			passengerUnit(i)
...
function passengerUnit(i){
	...
	trafficContract.methods.initPassenger(passengerId, web3.utils.asciiToHex(passengerPositionList[i].start)).send({from: passengerId, gas: 5000000,position:"wx411111111111",txtime:278000}).then(function(error, result){
