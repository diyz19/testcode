...
contract StoreTraffic {
    ...
    //new event
    event search_Event(bytes32 passengerId, bytes32 passengerGeohash);

    function send_VeID(bytes32 passengerId, bytes32 passengerGeohash) public {
        emit search_Event(passengerId, passengerGeohash);
    }
    //new event
    event return_Event(bytes32 passengerId, bytes32 passengerGeohash, bytes32 position, bytes32 vehicleId);

    function return_VeID(bytes32 passengerId, bytes32 passengerGeohash, bytes32 position, bytes32 vehicleId) public {
        emit return_Event(passengerId, passengerGeohash, position, vehicleId);
    }
