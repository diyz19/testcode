...
const VALUE1 = 100_0000_0000_00000
...
async function trans_tx(from_add ,to_add, web_to) {
	...
	web_to.eth.sendTransaction(
		...
		// { from: from_add, to: accountManagerW11, position: POSITION1, txtype: 1, txtime: Date.now()},
        function (err, res) {
			if (err) {
    ...
    await web3.eth.getBalance(from_add).then(function (result) {
        if (result != null) {
            const macc_outbal = result
            ...
            console.log("getAccountByRegion_Wrong!!!!!: ");
        }
    })
